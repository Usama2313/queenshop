...
app.get("/api/sync", async (req, res) => {
    try {
        ...
        res.json({ status: "ok", message: "BEST Store tables synced successfully!" });
    } catch (err) {
...
app.get("/api/create-admin", async (req, res) => {
    try {
        ...
        res.status(500).json({ status: "error", message: "Admin creation failed: " + err.message });
    }
});
...
app.get("/api/health", async (req, res) => {
    try {
        ...
        res.status(500).json({ status: "error", database: "disconnected", message: err.message, time: new Date() });
    }
});
